...
### 普通用户与 Tekton Task 权限验证

使用普通用户 `yzc-developer` 重新走同一套登录、token 获取、验签和鉴权验证链路，得到的关键 claims 如下：
...
```json
{
  ...
  "email": "yzc-developer",
  "email_verified": true,
  "preferred_username": "yzc-developer",
  "name": "YZC Developer",
  "groups": null,
  "roles": [
...
- 业务集群 direct API 仍然拒绝该 OIDC id_token。
- 平台代理 API 接受该 id_token，并把 Kubernetes 请求身份识别为 `user=yzc-developer`。
- 代理 API 返回的 groups 只有 `system:authenticated`，没有把 `roles=["namespace-developer-system"]` 直接当作 Kubernetes group 返回。

用平台代理 token 验证 `tasks.tekton.dev` 权限，结果如下：

| Namespace | get/list/watch | create/update/patch/delete | Conclusion |
| --- | --- | --- | --- |
| `yzc-dev` | yes | yes | 具备 Task 读写权限 |
| `yzc` | no | no | 不具备 Task 权限 |
| `default` | no | no | 不具备 Task 权限 |
| `tekton-pipelines` | no | no | 不具备 Task 权限 |

再用业务集群 direct API 创建 `SubjectAccessReview` 做语义验证，`--as=yzc-developer` 与平台代理结果一致：在 `yzc-dev` namespace 中对 `tekton.dev` API group 的 `tasks` 资源具备 `get/list/watch/create/update/patch/delete` 权限，在 `yzc`、`default`、`tekton-pipelines` 中不具备这些权限。使用 `--as=<id_token.sub>` 时，在 `yzc-dev` 中也不能命中这些 RBAC 权限。

进一步检查 RBAC 绑定，权限来源是：

- `yzc-dev` namespace 中存在绑定到 `User=yzc-developer` 的 `RoleBinding`。
- 该 `RoleBinding` 引用 `ClusterRole cpaas:namespace-developer`。
- `cpaas:namespace-developer` 包含 `apiGroups=["tekton.dev"]`、`resources` 包含 `tasks`、`verbs=["*"]` 的规则。
- 同一用户的 project/system/cluster 级绑定没有提供跨 namespace 的 `tekton.dev/tasks` 权限。

因此，普通用户验证进一步确认：当前平台的 `roles` claim 只是身份系统或平台层的角色声明，不能自动等同于 Kubernetes RBAC group。共享机制默认应把 `preferred_username` 映射为 Kubernetes `user` 来兼容当前 ACP 绑定；只有在调用方明确确认集群 RBAC 已按某个 claim 绑定时，才应把 `roles` 或 `groups` claim 映射为 SAR 中的 groups。
